#!/bin/bash
# Plot partial results from 2n2g lossy run (13/15 timesteps)
cd /u/$USER/GPUCompress

RESULTS=benchmarks/vpic-kokkos/results/eval_NX200_chunk16mb_ts15_lossy0.01
NRANKS=4
...
for r in rows:
    ...
    d['sum_wr'] += wr_val; d['sum_rd'] += rd_val
    d['sum_wr_sq'] += wr_val*wr_val; d['sum_rd_sq'] += rd_val*rd_val
    ...
    d['n'] += 1
first_orig = float(rows[0].get('orig_mib',0)) if 'orig_mib' in rows[0] else 0
if first_orig <= 0:
    for r in rows:
        if r['phase'] == 'no-comp':
            first_orig = int(r.get('file_bytes',0)) / (1024*1024); break
with open('$AGG_CSV','w') as f:
    ...
    for p,d in phases.items():
        n = d['n']
        if n == 0: continue
        avg_wr = d['sum_wr']/n; avg_rd = d['sum_rd']/n
        ...
        total_file_mib = d['sum_file_bytes']/(1024*1024); avg_file_mib = total_file_mib/n
        orig_mib = first_orig if first_orig>0 else avg_file_mib
        rat = (n*orig_mib)/total_file_mib if total_file_mib>0 else 0
        wr_mibps = (n*orig_mib)/(d['sum_wr']/1000.0) if d['sum_wr']>0 else 0
        rd_mibps = (n*orig_mib)/(d['sum_rd']/1000.0) if d['sum_rd']>0 else 0
        avg_comp=d['sum_comp']/n; avg_dec=d['sum_dec']/n
...
for (phase, ts), ranks in groups.items():
    if phase not in phases:
        phases[phase] = {'n_ts':0,'sum_agg_wr_mibps':0,'sum_agg_rd_mibps':0,
                         'sum_agg_wr_sq':0,'sum_agg_rd_sq':0,
                         'sum_orig_mib':0,'sum_file_mib':0,
                         'sum_max_wr_ms':0,'sum_max_rd_ms':0,
                         'avg_per_rank_wr':0,'avg_per_rank_rd':0,'n_per_rank':0}
    d = phases[phase]
    ...
    d['n_ts']+=1; d['sum_agg_wr_mibps']+=agg_wr; d['sum_agg_rd_mibps']+=agg_rd
    d['sum_agg_wr_sq']+=agg_wr*agg_wr; d['sum_agg_rd_sq']+=agg_rd*agg_rd
    d['sum_orig_mib']+=total_orig; d['sum_file_mib']+=total_file
    d['sum_max_wr_ms']+=max_wr; d['sum_max_rd_ms']+=max_rd
    for r in ranks:
        d['avg_per_rank_wr']+=float(r.get('write_mibps',0))
        d['avg_per_rank_rd']+=float(r.get('read_mibps',0))
        d['n_per_rank']+=1
with open('$AGG_MULTI','w') as f:
    f.write('n_ranks,phase,n_timesteps,avg_agg_write_mibps,avg_agg_read_mibps,'
    ...
    for phase in phases:
        d=phases[phase]; n=d['n_ts']
        if n==0: continue
        avg_wr=d['sum_agg_wr_mibps']/n; avg_rd=d['sum_agg_rd_mibps']/n
        wr_var=(d['sum_agg_wr_sq']/n-avg_wr**2)*n/(n-1) if n>1 else 0
        rd_var=(d['sum_agg_rd_sq']/n-avg_rd**2)*n/(n-1) if n>1 else 0
        wr_std=math.sqrt(wr_var) if wr_var>0 else 0
        rd_std=math.sqrt(rd_var) if rd_var>0 else 0
        orig=d['sum_orig_mib']/n; fmib=d['sum_file_mib']/n
        ...
        avg_mwr=d['sum_max_wr_ms']/n; avg_mrd=d['sum_max_rd_ms']/n
        f.write(f'{n_ranks},{phase},{n},{avg_wr:.1f},{avg_rd:.1f},'
                f'{wr_std:.1f},{rd_std:.1f},{pr_wr:.1f},{pr_rd:.1f},'
                f'{ratio:.4f},{orig:.2f},{fmib:.2f},{avg_mwr:.2f},{avg_mrd:.2f}\n')
