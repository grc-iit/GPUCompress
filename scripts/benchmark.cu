...
static const char* CSV_HEADER =
    ...
    "psnr_db,max_error,rmse,mean_abs_err,ssim,bit_rate,"
...
struct BenchmarkRow {
    ...
    std::string rmse_str;
    std::string mean_abs_err_str;
    std::string ssim_str;
    std::string bit_rate_str;
    double entropy;
    double mad;
...
static QualityMetrics compute_quality(const float* orig, const float* decomp,
    ...
    qm.mean_abs_err = sum_abs_err / nd;

    // SSIM (global single-window approximation)
    double mu_x = sum_x / nd, mu_y = sum_y / nd;
    double var_x = sum_x2 / nd - mu_x * mu_x;
    double var_y = sum_y2 / nd - mu_y * mu_y;
    double cov_xy = sum_xy / nd - mu_x * mu_y;
    double data_range = vmax - vmin;
    double L = (data_range > 0) ? data_range : 1.0;
    double c1 = (0.01 * L) * (0.01 * L);
    double c2 = (0.03 * L) * (0.03 * L);
    qm.ssim = ((2.0 * mu_x * mu_y + c1) * (2.0 * cov_xy + c2)) /
    ...
    const std::vector<BenchConfig>& configs)
{
    ...
    void *d_input = nullptr, *d_output = nullptr, *d_decomp = nullptr;
    cudaMalloc(&d_input, input_size);
    cudaMalloc(&d_output, max_out);
    cudaMalloc(&d_decomp, input_size);
    cudaMemcpy(d_input, raw.data(), input_size, cudaMemcpyHostToDevice);
    ...
    for (const auto& cfg : configs) {
        ...
        int rc = gpucompress_compress_gpu(
            d_input, input_size,
            d_output, &out_size,
            &c_cfg, &stats, nullptr);
        ...
        // Download compressed data for size and quality check
        if (rc == 0 && out_size > 0)
        ...
        int rc_d = gpucompress_decompress_gpu(
        ...
        // Download decompressed data for quality check
        if (rc_d == 0)
            cudaMemcpy(decomp_buf.data(), d_decomp, decomp_size, cudaMemcpyDeviceToHost);
        ...
        if (rc_d == 0 && cfg.quant_label == "linear") {
            ...
            if (decomp_elements == num_elements) {
                QualityMetrics qm = compute_quality(data, decomp_data, num_elements, compressed_size);
                ...
                row.mean_abs_err_str = fmt_double(qm.mean_abs_err, 6);
                row.ssim_str = fmt_double(qm.ssim, 8);
                row.bit_rate_str = fmt_double(qm.bit_rate, 4);
                ...
                row.mean_abs_err_str = "";
                row.ssim_str = "";
                row.bit_rate_str = (num_elements > 0) ? fmt_double(static_cast<double>(compressed_size) * 8.0 / num_elements, 4) : "";
            ...
            double br = (num_elements > 0) ? static_cast<double>(compressed_size) * 8.0 / num_elements : 32.0;
            if (match) {
                row.psnr_db = "inf";
                row.max_error_str = fmt_double(0.0, 1);
                row.rmse_str = fmt_double(0.0, 1);
                row.mean_abs_err_str = fmt_double(0.0, 1);
                row.ssim_str = fmt_double(1.0, 8);
                row.bit_rate_str = fmt_double(br, 4);
            } else {
