 ...
 * @brief Public C API wrappers and internal recording logic for diagnostics.
 *
 * All storage is owned by DiagnosticsStore::instance() (diagnostics_store.hpp).
 * This file only contains:
 *   - The public C API functions (thin delegates to the singleton)
 *   - recordChunkDiagnostic() — fills a gpucompress_chunk_diag_t from a
 *     ChunkDiagInput and hands it to the store
 *   - printNNDebugRanking() — stderr debug helper
 */
...
/* ============================================================
 * Public C API — chunk history
 * ============================================================ */

extern "C" void gpucompress_reset_chunk_history(void) {
    gpucompress::DiagnosticsStore::instance().reset();
    g_dbg_chunk = 0;
}
...
extern "C" int gpucompress_get_chunk_history_count(void) {
    return gpucompress::DiagnosticsStore::instance().count();
}

extern "C" int gpucompress_get_chunk_diag(int idx, gpucompress_chunk_diag_t *out) {
    if (!out) return -1;
    return gpucompress::DiagnosticsStore::instance().getDiag(idx, out);
}

extern "C" void gpucompress_record_chunk_decomp_ms(int idx, float ms) {
    gpucompress::DiagnosticsStore::instance().recordDecompMs(idx, ms);
}

extern "C" void gpucompress_record_chunk_vol_timing(int idx,
    float pool_acquire_ms, float d2h_copy_ms, float io_queue_wait_ms)
{
    gpucompress::DiagnosticsStore::instance().recordVolTiming(
        idx, pool_acquire_ms, d2h_copy_ms, io_queue_wait_ms);
}

extern "C" void gpucompress_record_chunk_s1_timing(int idx,
    float stats_malloc_ms, float stats_copy_ms, float wq_post_wait_ms)
{
    gpucompress::DiagnosticsStore::instance().recordS1Timing(
        idx, stats_malloc_ms, stats_copy_ms, wq_post_wait_ms);
}

/* ============================================================
 * Public C API — cache stats
 * ============================================================ */
...
extern "C" void gpucompress_reset_cache_stats(void) {
    gpucompress::DiagnosticsStore::instance().resetCacheStats();
}

extern "C" void gpucompress_get_cache_stats(int *hits, int *misses) {
    gpucompress::DiagnosticsStore::instance().getCacheStats(hits, misses);
}

/* ============================================================
 * Internal: recordChunkDiagnostic
 *
 * Builds a gpucompress_chunk_diag_t from a ChunkDiagInput and
 * appends it to DiagnosticsStore.  Called from
 * gpucompress_compress_with_action_gpu() after compression completes.
 * ============================================================ */
...
int recordChunkDiagnostic(const ChunkDiagInput& d)
{
    ...
    h.feat_eb_enc   = static_cast<float>(d.error_bound);
    h.feat_ds_enc   = static_cast<float>(d.input_size);
    /* P4 fix: use pre-copied host-side stats to avoid GPU stall under lock. */
    if (d.h_stats_valid) {
    ...
    /* diag_record_ms set post-hoc by caller via DiagnosticsStore::setDiagRecordMs() */

    int idx = DiagnosticsStore::instance().record(h);

    /* Debug: print per-chunk predicted vs actual summary.
     * Skip non-AUTO compressions (predicted_ratio==0). */
    if (g_debug_nn && h.predicted_ratio > 0.0f) {
        ...
        if (swapped) {
            ...
            fprintf(stderr, "[NN-DBG] [%s T=%d] Chunk %d | %s | "
                    ...
                    (double)h.predicted_decomp_time, 0.0,
                    (double)d.cost_model_error_pct * 100.0,
                    d.sgd_fired ? " [SGD]" : "",
