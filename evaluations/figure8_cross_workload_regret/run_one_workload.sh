...
# ── LAMMPS: MD phase transition (hot-sphere + NVT temperature ramp)
#
#    Physics: FCC lattice with hot sphere in cold matrix (initial velocities),
#             plus fix nvt so the target temperature ramps from LMP_T_COLD
#             to LMP_T_HOT over the whole run — traverses a continuous band
#             of thermodynamic states (microcanonical NVE does not).
#    Grid:    161³ FCC = 16.7M atoms × 3 comp × 4 bytes = 200 MB/field
#    Chunks:  200 MB / 4 MB = 50 chunks per write (positions only for ranking)
#    Writes:  default 200-step intervals × 8 writes = 1600 total steps
#
#    Why interval=200 (default): Finer thermal slices between dumps than 500;
#    override with LMP_SIM_INTERVAL. Shorter intervals (10-25) can make NN
#    adaptation too abrupt; 200-500 is a reasonable band for evolution
#    between GPUCompress writes.
#
#    Defaults T_cold=0.5, T_hot=2.5, Tdamp=1.0 (LJ units): bracket the
#    melting region (~0.7) while staying below T_hot≈10+ where LJ runs
#    can destabilize. Tdamp is in time units (same as timestep).
#
...
#
# SC26 LAMMPS defaults: multi-regime deck (LMP_DIVERSE=1) with the
# diversity-tuned thermodynamic parameters that produce real chunk-feature
# variation across dumps (verified in run 17387571).
#   ATOMS=80          → 4·80³ = 2,048,000 atoms (fcc, 4 atoms/cell)
#   TIMESTEPS=12      → 12 dumps = 2 full 6-stage cycles
#   SIM_INTERVAL=400  → 400 steps per stage (≈ 12 thermostat relaxations)
#   T_COLD=0.3, T_HOT=4.0, TDAMP=0.1
# Override any of these via env to run a different sizing.
LMP_ATOMS="${LMP_ATOMS:-80}"
LMP_TIMESTEPS="${LMP_TIMESTEPS:-12}"
LMP_SIM_INTERVAL="${LMP_SIM_INTERVAL:-400}"
LMP_WARMUP_STEPS="${LMP_WARMUP_STEPS:-0}"
LMP_T_HOT="${LMP_T_HOT:-4.0}"
LMP_T_COLD="${LMP_T_COLD:-0.3}"
LMP_TDAMP="${LMP_TDAMP:-0.1}"
# LMP_DIVERSE=1 (default) uses the multi-regime deck where each successive
# dump lives in a qualitatively different physical regime (cold solid →
# fast heat → compress → expand → quench → reinject → ...). This breaks
# the online-learning plateau seen with a single linear ramp. Set
# LMP_DIVERSE=0 to fall back to the legacy single-ramp deck.
LMP_DIVERSE="${LMP_DIVERSE:-1}"
...
run_lammps_sim() {
    ...
    note "Running LAMMPS simulation with $PHASE (atoms=$LMP_ATOMS^3, timesteps=$LMP_TIMESTEPS, diverse=$LMP_DIVERSE)"

    if [ "$LMP_DIVERSE" = "1" ]; then
        # ──────────────────────────────────────────────────────────────────
        # Multi-regime deck. Each "stage" lasts exactly LMP_SIM_INTERVAL
        # steps so it fills exactly one compressed dump. The stages walk
        # the system through 6 qualitatively different regimes; for
        # LMP_TIMESTEPS > 6 the cycle repeats with rotated phases so the
        # NN keeps seeing new (T, ρ, shock-front) corners.
        #
        #   stage 1: cold solid (T_cold)                — min entropy
        #   stage 2: fast heat to (T_hot+T_cold)/2      — cross freeze line
        #   stage 3: hot liquid + box compress 5%       — density jump up
        #   stage 4: hot liquid + box expand 10%        — density jump down
        #   stage 5: rapid heat to T_hot + reinject hot — second shock
        #   stage 6: deep quench T_hot → T_cold/2       — supercooled/glass
        # ──────────────────────────────────────────────────────────────────
        local n_stages=$LMP_TIMESTEPS
        local stage_steps=$LMP_SIM_INTERVAL
        local t_mid
        t_mid=$(awk -v a="$LMP_T_COLD" -v b="$LMP_T_HOT" 'BEGIN{printf "%.4f", (a+b)/2}')
        local t_quench
        t_quench=$(awk -v a="$LMP_T_COLD" 'BEGIN{printf "%.4f", a/2}')
        local t_reinject
        t_reinject=$(awk -v b="$LMP_T_HOT" 'BEGIN{printf "%.4f", b*2}')
...
fix             gpuc all gpucompress ${stage_steps} positions velocities forces
thermo          ${stage_steps}
timestep        0.003

# Optional warmup (no dumps yet — gpuc fix only fires inside the run loop)
LMPEOF

        if [ "$LMP_WARMUP_STEPS" -gt 0 ]; then
            cat >> "$work_dir/input.lmp" <<LMPEOF
fix             warm all nvt temp ${LMP_T_COLD} ${LMP_T_COLD} ${LMP_TDAMP}
unfix           gpuc
run             ${LMP_WARMUP_STEPS}
unfix           warm
fix             gpuc all gpucompress ${stage_steps} positions velocities forces
LMPEOF
        fi

        # Emit n_stages stages, cycling through the 6 regimes
        local s
        for ((s=1; s<=n_stages; s++)); do
            local kind=$(( (s - 1) % 6 + 1 ))
            cat >> "$work_dir/input.lmp" <<LMPEOF

# ───── stage $s / $n_stages  (regime $kind) ─────
LMPEOF
            case $kind in
                1)  # cold solid
                    cat >> "$work_dir/input.lmp" <<LMPEOF
fix             1 all nvt temp ${LMP_T_COLD} ${LMP_T_COLD} ${LMP_TDAMP}
run             ${stage_steps}
unfix           1
LMPEOF
                    ;;
                2)  # fast heat to mid temperature
                    cat >> "$work_dir/input.lmp" <<LMPEOF
fix             1 all nvt temp ${LMP_T_COLD} ${t_mid} ${LMP_TDAMP}
run             ${stage_steps}
unfix           1
LMPEOF
                    ;;
                3)  # hot liquid + box compress 5%
                    cat >> "$work_dir/input.lmp" <<LMPEOF
fix             1 all nvt temp ${t_mid} ${t_mid} ${LMP_TDAMP}
fix             def all deform 1 x scale 0.95 y scale 0.95 z scale 0.95 remap x
run             ${stage_steps}
unfix           def
unfix           1
LMPEOF
                    ;;
                4)  # hot liquid + box expand 10%
                    cat >> "$work_dir/input.lmp" <<LMPEOF
fix             1 all nvt temp ${t_mid} ${t_mid} ${LMP_TDAMP}
fix             def all deform 1 x scale 1.10 y scale 1.10 z scale 1.10 remap x
run             ${stage_steps}
unfix           def
unfix           1
LMPEOF
                    ;;
                5)  # rapid heat to T_hot + reinject hot region (second shock)
                    cat >> "$work_dir/input.lmp" <<LMPEOF
velocity        hot create ${t_reinject} 24680 loop geom
fix             1 all nvt temp ${t_mid} ${LMP_T_HOT} ${LMP_TDAMP}
run             ${stage_steps}
unfix           1
LMPEOF
                    ;;
                6)  # deep quench T_hot → T_cold/2 (supercooled / glass)
                    cat >> "$work_dir/input.lmp" <<LMPEOF
fix             1 all nvt temp ${LMP_T_HOT} ${t_quench} ${LMP_TDAMP}
run             ${stage_steps}
unfix           1
LMPEOF
                    ;;
        ...
        # Original single-ramp deck (default — backward compatible).
        cat > "$work_dir/input.lmp" <<LMPEOF
units           lj
atom_style      atomic
lattice         fcc 0.8442
...
fix             1 all nvt temp ${LMP_T_COLD} ${LMP_T_HOT} ${LMP_TDAMP}
...
write_run_metadata() {
    ...
    # ── Build the metadata file ──
    {
        ...
        if [ "$SKIP_LAMMPS" = "1" ]; then
            ...
            echo "### LAMMPS (molecular dynamics, hot-sphere + NVT ramp)"
            echo "Binary:          $LMP_BIN"
            echo ""
            echo "Simulation parameters:"
            ...
            echo "  T_HOT               = $LMP_T_HOT  (hot-sphere initial T; NVT ramp endpoint)"
            echo "  T_COLD              = $LMP_T_COLD  (cold lattice initial T; NVT ramp start)"
            echo "  TDAMP               = $LMP_TDAMP  (NVT thermostat damping, time units)"
            echo ""
        ...
        if [ "$SKIP_LAMMPS" != "1" ]; then
            ...
            echo "    LMP_T_HOT=$LMP_T_HOT LMP_T_COLD=$LMP_T_COLD LMP_TDAMP=$LMP_TDAMP \\"
        fi
        if [ "$SKIP_AI" != "1" ]; then
...
print_banner() {
    ...
    echo "LAMMPS        : $([ "$SKIP_LAMMPS" = "1" ] && echo "SKIP" || echo "atoms=${LMP_ATOMS}^3 T_hot=$LMP_T_HOT T_cold=$LMP_T_COLD tdamp=$LMP_TDAMP int=$LMP_SIM_INTERVAL diverse=$LMP_DIVERSE")"
