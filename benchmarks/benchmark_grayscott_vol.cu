 ...
 *   2. static       : GPU sim → H5Dwrite via VOL (lz4 + 4B shuffle)
...
#define TMP_STATIC   "/tmp/bm_gs_static.h5"
...
/* 3D chunked DCPL for static: lz4 + 4-byte shuffle */
static hid_t make_dcpl_static(int L, int chunk_z)
{
    hsize_t cdims[3] = { (hsize_t)L, (hsize_t)L, (hsize_t)chunk_z };
    hid_t dcpl = H5Pcreate(H5P_DATASET_CREATE);
    H5Pset_chunk(dcpl, 3, cdims);
...
/* ============================================================
 * Phase: VOL (static / nn / nn-rl / nn-rl+exp)
 * ============================================================ */
...
int main(int argc, char **argv)
{
    ...
    printf("╔═══════════════════════════════════════════════════════════════════════════╗\n");
    printf("║  Gray-Scott VOL Benchmark: No-Comp vs Static vs NN+SGD                  ║\n");
    printf("╚═══════════════════════════════════════════════════════════════════════════╝\n\n");
    ...
    hid_t dcpl_static = make_dcpl_static(L, chunk_z);
    rc = run_phase_vol(sim, steps, d_count,
                       n_floats, L, chunk_z,
                       "static", TMP_STATIC, dcpl_static,
                       &results[n_phases]);
