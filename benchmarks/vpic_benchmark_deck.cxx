 ...
 * @brief VPIC Benchmark Deck: No-Comp vs Static vs NN+SGD
 *
 ...
 *   2. static       : H5Dwrite via VOL (lz4 + 4B shuffle)
 ...
 *     -DENABLE_KOKKOS_CUDA=ON -DBUILD_INTERNAL_KOKKOS=ON \
 *     -DENABLE_KOKKOS_OPENMP=OFF -DCMAKE_CXX_STANDARD=17 \
 *     -DKokkos_ARCH_AMPERE80=ON -DKokkos_ENABLE_CUDA_LAMBDA=ON \
 *     -DUSER_DECKS="/u/imuradli/GPUCompress/tests/benchmarks/vpic_benchmark_deck.cxx" \
 *     '-DCMAKE_CXX_FLAGS=-I/u/imuradli/GPUCompress/include -I/u/imuradli/GPUCompress/examples -I/tmp/hdf5-install/include' \
 *     '-DCMAKE_EXE_LINKER_FLAGS=-L/u/imuradli/GPUCompress/build -lgpucompress -lH5VLgpucompress -lH5Zgpucompress -L/tmp/hdf5-install/lib -lhdf5 -L/tmp/lib -lnvcomp'
 ...
 *   mpirun -np 1 ./build-benchmark/vpic_benchmark_deck
 *
 * OUTPUT:
...
#define TMP_STATIC   "/tmp/bm_vpic_static.h5"
...
begin_initialization {
    ...
    global->chunk_bytes   = 64 * 1024 * 1024;  // 64 MB chunks
...
begin_diagnostics {
    ...
    sim_log("");
    sim_log("╔═══════════════════════════════════════════════════════════════════════════╗");
    sim_log("║  VPIC Benchmark: No-Comp vs Static vs NN+SGD (Real Harris Sheet Data)   ║");
    sim_log("╚═══════════════════════════════════════════════════════════════════════════╝");
    sim_log("");
    ...
    sim_log("── Phase 2/5: static (VOL, lz4+4B-shuffle) ──────────────────");
    {
