                          ...
                          RunResult *result)
{
    ...
    /* For convergence tracking: running MAPE */
    double running_mape_sum = 0;
    int running_mape_n = 0;
    int convergence_chunk = 0;  /* first chunk where running MAPE < 10% */
    ...
    for (int i = 0; i < n_hist; i++) {
        ...
        if (d.predicted_ratio > 0 && d.actual_ratio > 0) {
            ...
            running_mape_sum += chunk_mape;
            running_mape_n++;
            if (convergence_chunk == 0 && running_mape_n >= 5) {
                double running = running_mape_sum / running_mape_n;
                if (running < 0.10) {
...
int main(int argc, char **argv)
{
    ...
    if (!weights_path) {
        fprintf(stderr, "Usage: %s <weights.nnwt> [--L N] [--chunk-mb N] [--error-bound E] [--quick]\n",
                argv[0]);
        return 1;
    }
    ...
    float sgd_lrs_full[]  = {0.1f, 0.2f, 0.4f, 0.8f};
    float sgd_mapes_full[] = {0.10f, 0.20f, 0.30f};
    float sgd_lrs_quick[]  = {0.05f, 0.2f, 0.8f};
    float sgd_mapes_quick[] = {0.10f, 0.30f};
